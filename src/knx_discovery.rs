...
/// ```
pub async fn discover_gateway(stack: &Stack<'static>, timeout: Duration) -> Option<GatewayInfo> {
    ...
    let multicast_endpoint =
        IpEndpoint::new(embassy_net::Ipv4Address::new(a, b, c, d).into(), KNX_PORT);

    // Try to send (ignore errors, UDP is best-effort)
    let _ = socket
        .send_to(&request_buf[..request_len], multicast_endpoint)
        .await;
    ...
    if let Some(_gateway_addr) = local_config.gateway {
        ...
        let broadcast_endpoint =
            IpEndpoint::new(embassy_net::Ipv4Address::new(a, b, c, d).into(), KNX_PORT);
        let _ = socket
            .send_to(&request_buf[..request_len], broadcast_endpoint)
            .await;
    }
...
mod tests {
    ...
    fn test_calculate_broadcast() {
        ...
        // /16 network
        assert_eq!(calculate_broadcast([10, 0, 5, 10], 16), [10, 0, 255, 255]);
    }
}
