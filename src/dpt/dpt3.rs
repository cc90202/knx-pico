...
mod tests {
    ...
    fn test_encode_break() {
        ...
        let len = Dpt3::Dimming
            .encode((false, StepCode::Break), &mut buf)
            .unwrap();
        assert_eq!(len, 1);
        ...
        let len = Dpt3::Dimming
            .encode((true, StepCode::Break), &mut buf)
            .unwrap();
        assert_eq!(len, 1);
    ...
    fn test_encode_dimming_decrease() {
        ...
        // 0b0000_0001 = 0x01
        let len = Dpt3::Dimming
            .encode((false, StepCode::Intervals1), &mut buf)
            .unwrap();
        assert_eq!(len, 1);
        ...
        // 0b0000_0011 = 0x03
        let len = Dpt3::Dimming
            .encode((false, StepCode::Intervals4), &mut buf)
            .unwrap();
        assert_eq!(len, 1);
    ...
    fn test_encode_dimming_increase() {
        ...
        // 0b0000_1001 = 0x09
        let len = Dpt3::Dimming
            .encode((true, StepCode::Intervals1), &mut buf)
            .unwrap();
        assert_eq!(len, 1);
        ...
        // 0b0000_1011 = 0x0B
        let len = Dpt3::Dimming
            .encode((true, StepCode::Intervals4), &mut buf)
            .unwrap();
        assert_eq!(len, 1);
    ...
    fn test_encode_blind_up() {
        ...
        // 0b0000_0010 = 0x02
        let len = Dpt3::Blind
            .encode((false, StepCode::Intervals2), &mut buf)
            .unwrap();
        assert_eq!(len, 1);
    ...
    fn test_encode_blind_down() {
        ...
        // 0b0000_1100 = 0x0C
        let len = Dpt3::Blind
            .encode((true, StepCode::Intervals8), &mut buf)
            .unwrap();
        assert_eq!(len, 1);
    ...
    fn test_round_trip() {
        ...
        for (control, stepcode) in &test_cases {
            // Encode
            let len = Dpt3::Dimming
                .encode((*control, *stepcode), &mut buf)
                .unwrap();
            assert_eq!(len, 1);
